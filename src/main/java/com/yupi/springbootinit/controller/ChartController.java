...
//    private RedisLimiterManager redisLimiterManager;
    ...
    @PostMapping("/gen")
    public BaseResponse<BiResponse> getChartAI(@RequestPart("file") MultipartFile multipartFile,
        ...
        User loginUser = userService.getLoginUser(request);
        //限流判断
        //redisLimiterManager.doRateLimit("getChartAI_" + loginUser.getId());

        // AI 编号
        long biModelId = 1665315729097916417L;
        ...
        chart.setUserId(loginUser.getId());
        boolean saveRun = chartService.save(chart);
        ThrowUtils.throwIf(!saveRun,ErrorCode.SYSTEM_ERROR,"运行出错");
    ...
    public BaseResponse<BiResponse> getChartAIAsync(@RequestPart("file") MultipartFile multipartFile,
                                           GenChartByAiRequest genChartByAiRequest, HttpServletRequest request) {
        String name = genChartByAiRequest.getName();
        String goal = genChartByAiRequest.getGoal();
        String chartType = genChartByAiRequest.getChartType();
